...
test("authenticated clinical API persists patients and encounters", async t => {
  ...
  assert.equal(arrivalPayload.workflow.arrivals.length, 1);
  assert.equal(arrivalPayload.workflow.arrivals[0].priority, "Urgent");
  const duplicateArrival = await fetch(`${base}/api/arrivals`, { method: "POST", headers, body: JSON.stringify({ patientUuid: created.patient.uuid, clinic: "Outpatient", priority: "Routine", reason: "Duplicate check-in" }) });
  assert.equal(duplicateArrival.status, 409);
  const startedArrival = await fetch(`${base}/api/arrivals/${arrivalPayload.arrival.uuid}`, { method: "PATCH", headers, body: JSON.stringify({ status: "In progress" }) }).then(r => r.json());
  assert.equal(startedArrival.workflow.arrivals[0].status, "In progress");
  const imagingResponse = await fetch(`${base}/api/patients/${created.patient.uuid}/imaging-orders`, { method: "POST", headers, body: JSON.stringify({ serviceName: "Chest X-ray", priority: "Urgent", indication: "Persistent cough with focal chest signs" }) });
  assert.equal(imagingResponse.status, 201);
  const imagingPayload = await imagingResponse.json();
  assert.equal(imagingPayload.workflow.imagingWorklist.length, 1);
  assert.equal(imagingPayload.workflow.invoices[0].items.some(item => item.source_type === "imaging" && item.unit_price === 1800), true);
  const cancelledImaging = await fetch(`${base}/api/imaging-orders/${imagingPayload.order.uuid}/status`, { method: "PATCH", headers, body: JSON.stringify({ status: "Cancelled" }) }).then(r => r.json());
  assert.equal(cancelledImaging.workflow.imagingWorklist.length, 0);
  assert.equal(cancelledImaging.workflow.invoices.length, 0);
  ...
  const encounterResponse = await fetch(`${base}/api/encounters`, { method: "POST", headers, body: JSON.stringify({ patientUuid: created.patient.uuid, diagnosis: "Test diagnosis", vitals: { temperature: "37 C" }, summary: "Test visit", clinicData: { clinic: "DM", values: { glucoseReading: "8.2 mmol/L", dmAdherence: "Good", hypoglycaemia: "No", footScreen: "Normal", dmGoal: "Walk for 30 minutes five days weekly" } } }) });
  assert.equal(encounterResponse.status, 201);
  const encounterPayload = await encounterResponse.json();
  assert.equal(encounterPayload.encounter.clinicData.clinic, "DM");
  assert.equal(encounterPayload.encounter.clinicData.values.footScreen, "Normal");
  const completedQueue = await fetch(`${base}/api/workflow`, { headers }).then(r => r.json());
  assert.equal(completedQueue.workflow.arrivals.length, 0);
