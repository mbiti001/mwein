...
test("authenticated clinical API persists patients and encounters", async t => {
  ...
  assert.equal(bootstrap.staff.length, 1);
  assert.equal(bootstrap.staff[0].role, "Admin");
  assert.equal(bootstrap.staffRoles.length, 6);
  ...
  assert.equal(bootstrap.metrics.patientsToday, 4);
  assert.equal(bootstrap.metrics.dataQuality.score, 100);
  assert.equal(bootstrap.metrics.visitsByDay.length, 7);
  const weakStaffPassword = await fetch(`${base}/api/staff`, { method: "POST", headers, body: JSON.stringify({ displayName: "Alice Muli", email: "alice@mwein.local", initials: "AM", role: "Nurse", temporaryPassword: "too-short" }) });
  assert.equal(weakStaffPassword.status, 422);
  const staffPassword = "NurseTemporary2026!";
  const staffResponse = await fetch(`${base}/api/staff`, { method: "POST", headers, body: JSON.stringify({ displayName: "Alice Muli", email: "alice@mwein.local", initials: "AM", role: "Nurse", temporaryPassword: staffPassword }) });
  assert.equal(staffResponse.status, 201);
  const staffPayload = await staffResponse.json();
  const nurse = staffPayload.staff.find(member => member.email === "alice@mwein.local");
  assert.equal(nurse.role, "Nurse");
  assert.equal((await fetch(`${base}/api/staff`, { method: "POST", headers, body: JSON.stringify({ displayName: "Alice Muli", email: "alice@mwein.local", initials: "AM", role: "Nurse", temporaryPassword: staffPassword }) })).status, 409);
  const nurseLogin = await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: nurse.email, password: staffPassword }) });
  const nurseCookie = nurseLogin.headers.get("set-cookie").split(";")[0];
  assert.equal((await nurseLogin.clone().json()).user.mustChangePassword, true);
  assert.equal((await fetch(`${base}/api/staff`, { headers: { cookie: nurseCookie } })).status, 403);
  const roleUpdate = await fetch(`${base}/api/staff/${nurse.uuid}`, { method: "PATCH", headers, body: JSON.stringify({ role: "Lab", active: true }) });
  assert.equal(roleUpdate.status, 200);
  assert.equal((await fetch(`${base}/api/auth/me`, { headers: { cookie: nurseCookie } })).status, 401);
  const labLogin = await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: nurse.email, password: staffPassword }) });
  const labCookie = labLogin.headers.get("set-cookie").split(";")[0];
  const labUser = await labLogin.json();
  assert.equal(labUser.user.role, "Lab");
  assert.equal(labUser.user.mustChangePassword, true);
  const personalPassword = "AlicePersonal2026!";
  assert.equal((await fetch(`${base}/api/auth/password`, { method: "POST", headers: { "content-type": "application/json", cookie: labCookie }, body: JSON.stringify({ currentPassword: staffPassword, newPassword: personalPassword }) })).status, 200);
  const labBootstrap = await fetch(`${base}/api/bootstrap`, { headers: { cookie: labCookie } }).then(response => response.json());
  assert.equal(labBootstrap.patients.length, 0);
  assert.equal(labBootstrap.audit.length, 0);
  assert.equal(labBootstrap.formulary.length, 0);
  assert.equal(labBootstrap.workflow.labCatalog.length, 6);
  assert.equal(labBootstrap.workflow.pharmacyQueue.length, 0);
  assert.equal(labBootstrap.workflow.invoices.length, 0);
  assert.equal(labBootstrap.metrics.totalPatients, 0);
  assert.equal((await fetch(`${base}/api/staff`, { headers: { cookie: labCookie } })).status, 403);
  assert.equal((await fetch(`${base}/api/prescription-plugins`, { headers: { cookie: labCookie } })).status, 403);
  assert.equal((await fetch(`${base}/api/patients`, { method: "POST", headers: { "content-type": "application/json", cookie: labCookie }, body: JSON.stringify({}) })).status, 403);
  const replacementPassword = "Replacement2026!Secure";
  assert.equal((await fetch(`${base}/api/staff/${nurse.uuid}/password`, { method: "PUT", headers, body: JSON.stringify({ temporaryPassword: replacementPassword }) })).status, 200);
  assert.equal((await fetch(`${base}/api/auth/me`, { headers: { cookie: labCookie } })).status, 401);
  assert.equal((await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: nurse.email, password: personalPassword }) })).status, 401);
  const replacementLogin = await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: nurse.email, password: replacementPassword }) });
  assert.equal(replacementLogin.status, 200);
  assert.equal((await replacementLogin.json()).user.mustChangePassword, true);
  assert.equal((await fetch(`${base}/api/staff/${nurse.uuid}`, { method: "PATCH", headers, body: JSON.stringify({ role: "Lab", active: false }) })).status, 200);
  assert.equal((await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: nurse.email, password: replacementPassword }) })).status, 401);
  assert.equal((await fetch(`${base}/api/staff/${bootstrap.user.uuid}`, { method: "PATCH", headers, body: JSON.stringify({ role: "Clinician", active: true }) })).status, 409);
  ...
  const afterRegistrationMetrics = await fetch(`${base}/api/metrics`, { headers }).then(r => r.json());
  assert.equal(afterRegistrationMetrics.metrics.totalPatients, 5);
  assert.equal(afterRegistrationMetrics.metrics.patientsToday, 5);
  assert.equal(afterRegistrationMetrics.metrics.dataQuality.missingNextOfKin, 1);
  ...
  assert.equal(finalMetrics.metrics.dispensedPrescriptions, 1);
  assert.equal(finalMetrics.metrics.openInvoices, 0);
  assert.equal(finalMetrics.metrics.readyInvoices, 1);
  assert.equal(finalMetrics.metrics.dataQuality.missingDiagnosisCodes, 1);
  assert.equal(finalMetrics.metrics.dataQuality.score, 71);
