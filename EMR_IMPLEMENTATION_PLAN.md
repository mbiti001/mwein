...
The current compatibility pilot uses an authenticated Node service and SQLite database for patients, encounters, prescriptions, laboratory orders, invoices, audit events, and live operational metrics. Local development may seed demonstration records on the server, while production rejects demo seeding. It is not the canonical product or a certified production medical record system; the supported Next.js/PostgreSQL application is in `platform/`.
