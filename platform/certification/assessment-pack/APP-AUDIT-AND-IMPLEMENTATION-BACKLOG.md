...
Current follow-up: [remaining-work status](REMAINING-WORK-STATUS.md) supersedes implementation states below. The user authorized resuming MFA; it is now implemented in the remediation source with Google Authenticator-compatible codes and governed recovery. Audit inventory coverage and missing operational disclosures are addressed in this follow-up. Production/independent acceptance remains outstanding.
...
| APP-06 | P2 / implemented in source | `scripts/audit-coverage.mjs` checks specific function-name strings in six path families, not all access paths/callees | Reviewed route/control inventory plus behavioral tests; avoid counting existing audited paths as gaps |
...
| APP-08 | P1 / partially implemented | Local manual aggregate draft/review/approval and immutable correction history now implemented on remediation branch; national mapping/submission/acknowledgement remain absent | Approved dataset mapping, source reconciliation, delivery lifecycle and acceptance evidence |
| APP-09 | P1 / partially implemented | Local case/event capture, review, duplicate linkage and immutable manual notification history implemented on remediation branch | Approved case definitions, routing, escalation, weekly returns and genuine receiving-service acknowledgement remain outstanding |
...
| APP-11 | P1 / implemented in source | Native authenticator MFA and governed recovery resumed and implemented; optional OIDC acceptance remains incomplete | Roll out, enroll staff and approve operational recovery evidence |
...
- **MFA (APP-11):** resumed by the user after the Google/DHA options review; see the current status above. Production rollout, staff enrollment and approved recovery evidence remain outstanding.
