...
const ready = { status: "DISCHARGED", clinicallyClosedAt: new Date(), dispositionRecord: { outcome: "RECOVERED" }, encounters: [{ status: "SIGNED" }], orders: [{ status: "COMPLETED", displayName: "FBC" }], invoice: { status: "PAID", items: [{ quantity: 1, unitPrice: 500 }], payments: [{ status: "CONFIRMED", amount: 500 }], claims: [] } };

describe("visit completion", () => {
  it("blocks administrative completion without a clinical outcome", () => expect(visitCompletionBlockers({ ...ready, dispositionRecord: null })).toContain("Clinical discharge disposition is not recorded"));
