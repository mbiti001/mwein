...
export function proxy(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === "production" && !configuredOrigin)
    return NextResponse.json({ error: "Application origin is not configured" }, { status: 503, headers: { "Cache-Control": "private, no-store" } });

  let expected: string;
  try { expected = new URL(configuredOrigin || request.nextUrl.origin).origin; }
  catch { return NextResponse.json({ error: "Application origin is not valid" }, { status: 503, headers: { "Cache-Control": "private, no-store" } }); }
  ...
  if (!suppliedOrigin || suppliedOrigin !== expected || (fetchSite && !["same-origin", "none"].includes(fetchSite)))
    return NextResponse.json({ error: "Request origin was not accepted" }, { status: 403, headers: { "Cache-Control": "private, no-store" } });
  return next();
}
