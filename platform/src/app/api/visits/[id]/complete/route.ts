  ...
  try {
    ...
    const result = await db.$transaction(async tx => {
      const visit = await tx.visit.findFirst({
        ...
        include: { encounters: true, orders: true, invoice: { include: { items: true, payments: true, claims: true } } },
      });
      ...
      await appendAudit(tx, { userId: user.id, action: "VISIT_COMPLETED", entityType: "Visit", entityId: id, beforeHash: visit.status, afterHash: completedAt.toISOString() });
      return completed;
    });
