...
export async function GET() {
  try {
    ...
    const [appointments, referrals, specialty, encounters] = await Promise.all([
      ...
      db.referral.findMany({ where: { facilityId: user.facilityId, status: { in: ["SENT", "ACCEPTED", "ATTENDED"] }, updatedAt: { lt: referralStale } }, select: { id: true, referralNumber: true, receivingFacility: true, status: true, updatedAt: true, patient: { select: { id: true, patientNumber: true, fullName: true } } }, orderBy: { updatedAt: "asc" }, take: 100 }),
      db.servicePointRecord.findMany({ where: { encounter: { visit: { facilityId: user.facilityId } }, followUpAt: { lte: horizon } }, select: { id: true, servicePoint: true, followUpAt: true, encounter: { select: { visit: { select: { id: true, visitNumber: true, patient: { select: { id: true, patientNumber: true, fullName: true } } } } } } }, orderBy: { followUpAt: "asc" }, take: 100 }),
    ...
    await recordDisclosure(user, "FOLLOW_UPS", [...appointments, ...referrals, ...specialty, ...clinical].map(item => item.id));
    return privateJson({ generatedAt: now.toISOString(), horizon: horizon.toISOString(), summary: { missedAppointments: appointments.length, referralFeedback: referrals.length, specialtyReviews: specialty.length, clinicalFollowUps: clinical.length }, worklists: { appointments, referrals, specialty: specialty.map(item => ({ ...item, visitId: item.encounter.visit.id, visitNumber: item.encounter.visit.visitNumber, patient: item.encounter.visit.patient, encounter: undefined })), clinical } });
