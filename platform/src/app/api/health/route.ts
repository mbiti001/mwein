...
export async function GET() {
  try {
    ...
    `;
    return NextResponse.json({ status: "ok", database: "connected", migration: migration[0]?.migration_name ?? "none", release: releaseIdentity(), time: new Date().toISOString() }, { headers: { "cache-control": "no-store" } });
  } catch {
