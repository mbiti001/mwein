...
export async function GET() {
  try {
    ...
    return NextResponse.json({ suppliers, stores, purchaseOrders, items, batches, pendingCounts, emergencyAdjustments, movements, currentUserId: user.id });
...
async function receive(tx: Tx, user: User, input: Extract<z.infer<typeof actionSchema>, { action: "RECEIVE" }>) {
  ...
  const batch = existing ? await tx.inventoryBatch.update({ where: { id: existing.id }, data: { quantityReceived: { increment: input.quantity }, quantityAvailable: { increment: input.quantity }, unitCost: line.unitCost, active: true } }) : await tx.inventoryBatch.create({ data: { catalogItemId: line.catalogItemId, batchNumber: input.batchNumber, expiryDate: input.expiryDate, quantityReceived: input.quantity, quantityAvailable: input.quantity, unitCost: line.unitCost } });
  ...
  await tx.stockMovement.create({ data: { batchId: batch.id, userId: user.id, destinationStoreId: store.id, type: "RECEIPT", quantity: input.quantity, balanceAfter: batch.quantityAvailable, destinationBalanceAfter: destinationBalance.quantity, reason: `${line.purchaseOrder.orderNumber} · GRN to ${store.name}` } });
...
async function approveCount(tx: Tx, user: User, eventId: string) {
  ...
  await tx.stockMovement.create({ data: { batchId: event.batchId, userId: user.id, sourceStoreId: event.storeId, type: "ADJUSTMENT", quantity: variance, balanceAfter: batch.quantityAvailable, sourceBalanceAfter: event.quantity, reason: event.reason } });
...
async function emergencyAdjust(tx: Tx, user: User, input: Extract<z.infer<typeof actionSchema>, { action: "EMERGENCY_ADJUST" }>) {
  ...
  await tx.stockMovement.create({ data: { batchId: input.batchId, userId: user.id, sourceStoreId: input.storeId, type: "EMERGENCY_ADJUSTMENT", quantity: variance, balanceAfter: batch.quantityAvailable, sourceBalanceAfter: input.newQuantity, reason: input.reason } });
  await operation(tx, user, input.idempotencyKey, input.action, "InventoryControlEvent", record.id);
