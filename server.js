...
const DATA_DIR = process.env.DATA_DIR || (IS_VERCEL ? "/tmp/mwein-emr" : join(ROOT, "data"));
...
const vercelOrigin = process.env.VERCEL_URL || process.env.VERCEL_PROJECT_PRODUCTION_URL;
const PUBLIC_ORIGIN = (process.env.PUBLIC_ORIGIN || (vercelOrigin ? `https://${vercelOrigin}` : "")).replace(/\/$/, "");
const TRUST_PROXY = process.env.TRUST_PROXY === "true" || IS_VERCEL;
...
const IMAGING_CATALOG = [
  { service: "Chest X-ray", modality: "X-ray", price: 1800 },
  { service: "Abdominal ultrasound", modality: "Ultrasound", price: 2500 },
  { service: "Obstetric ultrasound", modality: "Ultrasound", price: 2800 },
  { service: "Pelvic ultrasound", modality: "Ultrasound", price: 2500 },
  { service: "Limb X-ray", modality: "X-ray", price: 1600 },
  { service: "ECG", modality: "Cardiology", price: 1200 }
];
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS encounters (
    ...
    clinic_data_json TEXT NOT NULL DEFAULT '{}',
  ...
  CREATE TABLE IF NOT EXISTS arrivals (
    uuid TEXT PRIMARY KEY,
    patient_uuid TEXT NOT NULL REFERENCES patients(uuid),
    clinic TEXT NOT NULL DEFAULT 'Outpatient',
    priority TEXT NOT NULL DEFAULT 'Routine' CHECK(priority IN ('Routine', 'Priority', 'Urgent', 'Emergency')),
  ...
  CREATE TABLE IF NOT EXISTS invoice_items (
    ...
    source_type TEXT NOT NULL CHECK(source_type IN ('lab', 'prescription', 'imaging')),
  ...
  CREATE INDEX IF NOT EXISTS idx_arrivals_queue ON arrivals(status, priority, arrived_at);
  ...
  CREATE INDEX IF NOT EXISTS idx_imaging_orders_patient ON imaging_orders(patient_uuid, created_at DESC);
...
const invoiceItemsSql = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'invoice_items'").get()?.sql || "";
if (!invoiceItemsSql.includes("'imaging'")) {
  db.exec(`
    ...
    INSERT INTO invoice_items SELECT * FROM invoice_items_legacy;
    DROP TABLE invoice_items_legacy;
...
const encounterColumns = db.prepare("PRAGMA table_info(encounters)").all().map(column => column.name);
if (!encounterColumns.includes("clinic_data_json")) db.exec("ALTER TABLE encounters ADD COLUMN clinic_data_json TEXT NOT NULL DEFAULT '{}'");
...
function workflowState(patientUuid = "") {
  ...
  const imagingSql = `SELECT o.*, p.name AS patient_name, p.identifier AS patient_identifier FROM imaging_orders o JOIN patients p ON p.uuid = o.patient_uuid WHERE 1 = 1${patientUuid ? " AND o.patient_uuid = ?" : ""} ORDER BY o.created_at DESC`;
  const args = patientUuid ? [patientUuid] : [];
  const arrivalSql = `SELECT a.*, p.name AS patient_name, p.identifier AS patient_identifier, p.age, p.sex
    FROM arrivals a JOIN patients p ON p.uuid = a.patient_uuid
    WHERE a.status IN ('Waiting', 'In progress')${patientUuid ? " AND a.patient_uuid = ?" : ""}
    ORDER BY CASE a.priority WHEN 'Emergency' THEN 0 WHEN 'Urgent' THEN 1 WHEN 'Priority' THEN 2 ELSE 3 END, a.arrived_at`;
  const labOrders = db.prepare(labSql).all(...args);
  const pharmacyOrders = db.prepare(pharmacySql).all(...args);
  const imagingOrders = db.prepare(imagingSql).all(...args);
  return {
    arrivals: db.prepare(arrivalSql).all(...args),
    labCatalog: LAB_CATALOG,
    imagingCatalog: IMAGING_CATALOG,
    ...
    imagingOrders,
    imagingWorklist: imagingOrders.filter(order => !["Completed", "Cancelled"].includes(order.status)),
...
function operationalMetrics() {
  ...
  return {
    ...
    visitsByDay: Array.from({ length: 7 }, (_, index) => kenyaDayWindow(6 - index)).map(day => ({
      label: day.label,
...
async function api(req, res, url, requestId) {
  ...
  if (req.method === "POST" && url.pathname === "/api/arrivals") {
    if (!can(authenticatedUser, ["Clinician", "Nurse"])) return sendJson(res, 403, { error: "Your role cannot check in patients" }, requestId);
    const body = await readJson(req);
    const patientUuid = text(body.patientUuid, 80);
    const clinic = text(body.clinic || "Outpatient", 80);
    ...
    if (!["Outpatient", "ANC", "HTN", "DM", "Paediatrics", "Emergency", "Other"].includes(clinic)) return sendJson(res, 422, { error: "Select a valid clinic" }, requestId);
    if (!["Routine", "Priority", "Urgent", "Emergency"].includes(priority)) return sendJson(res, 422, { error: "Select a valid urgency" }, requestId);
    const existing = db.prepare("SELECT uuid FROM arrivals WHERE patient_uuid = ? AND status IN ('Waiting', 'In progress') LIMIT 1").get(patientUuid);
    if (existing) return sendJson(res, 409, { error: "This patient already has an active arrival", arrivalUuid: existing.uuid }, requestId);
    const uuid = randomUUID();
    db.prepare("INSERT INTO arrivals (uuid, patient_uuid, clinic, priority, reason, arrived_at) VALUES (?, ?, ?, ?, ?, ?)")
      .run(uuid, patientUuid, clinic, priority, reason, now());
    audit(authenticatedUser.initials, "Patient checked in", patientUuid, `${clinic} · ${priority}`, requestId);
    return sendJson(res, 201, { arrival: db.prepare("SELECT * FROM arrivals WHERE uuid = ?").get(uuid), workflow: workflowState(), audit: listAudit() }, requestId);
  }
  const arrivalMatch = url.pathname.match(/^\/api\/arrivals\/([^/]+)$/);
  if (req.method === "PATCH" && arrivalMatch) {
    if (!can(authenticatedUser, ["Clinician", "Nurse"])) return sendJson(res, 403, { error: "Your role cannot update the arrival queue" }, requestId);
    const uuid = decodeURIComponent(arrivalMatch[1]);
    const arrival = db.prepare("SELECT * FROM arrivals WHERE uuid = ?").get(uuid);
    if (!arrival) return sendJson(res, 404, { error: "Arrival not found" }, requestId);
    const status = text((await readJson(req)).status, 30);
    if (!["Waiting", "In progress", "Completed", "Cancelled"].includes(status)) return sendJson(res, 422, { error: "Invalid arrival status" }, requestId);
    const timestamp = now();
    db.prepare("UPDATE arrivals SET status = ?, started_at = CASE WHEN ? = 'In progress' THEN COALESCE(started_at, ?) ELSE started_at END, completed_at = CASE WHEN ? IN ('Completed', 'Cancelled') THEN ? ELSE completed_at END WHERE uuid = ?")
      .run(status, status, timestamp, status, timestamp, uuid);
    audit(authenticatedUser.initials, "Arrival status updated", arrival.patient_uuid, status, requestId);
    return sendJson(res, 200, { workflow: workflowState(), audit: listAudit() }, requestId);
  }
  ...
  const patientImagingMatch = url.pathname.match(/^\/api\/patients\/([^/]+)\/imaging-orders$/);
  if (req.method === "POST" && patientImagingMatch) {
    if (!can(authenticatedUser, ["Clinician"])) return sendJson(res, 403, { error: "Only clinicians can request imaging" }, requestId);
    const patientUuid = decodeURIComponent(patientImagingMatch[1]);
    if (!db.prepare("SELECT 1 FROM patients WHERE uuid = ?").get(patientUuid)) return sendJson(res, 404, { error: "Patient not found" }, requestId);
    const body = await readJson(req);
    const serviceName = text(body.serviceName, 120);
    const indication = text(body.indication, 1000);
    const priority = text(body.priority || "Routine", 30);
    const catalogItem = IMAGING_CATALOG.find(item => item.service === serviceName);
    if (!catalogItem || !indication || !["Routine", "Urgent"].includes(priority)) return sendJson(res, 422, { error: "Imaging service, clinical indication, and priority are required" }, requestId);
    const uuid = randomUUID();
    const timestamp = now();
    db.exec("BEGIN");
    try {
      db.prepare("INSERT INTO imaging_orders (uuid, patient_uuid, service_name, modality, indication, priority, status, price, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'Requested', ?, ?, ?)")
        .run(uuid, patientUuid, catalogItem.service, catalogItem.modality, indication, priority, catalogItem.price, timestamp, timestamp);
      addInvoiceItem(patientUuid, "imaging", uuid, catalogItem.service, 1, catalogItem.price);
      audit(authenticatedUser.initials, "Imaging requested", catalogItem.service, "Invoice line added", requestId);
      db.exec("COMMIT");
    } catch (error) { db.exec("ROLLBACK"); throw error; }
    return sendJson(res, 201, { order: db.prepare("SELECT * FROM imaging_orders WHERE uuid = ?").get(uuid), workflow: workflowState(patientUuid), audit: listAudit() }, requestId);
  }
  const imagingStatusMatch = url.pathname.match(/^\/api\/imaging-orders\/([^/]+)\/status$/);
  if (req.method === "PATCH" && imagingStatusMatch) {
    const uuid = decodeURIComponent(imagingStatusMatch[1]);
    const order = db.prepare("SELECT patient_uuid, service_name FROM imaging_orders WHERE uuid = ?").get(uuid);
    if (!order) return sendJson(res, 404, { error: "Imaging request not found" }, requestId);
    const status = text((await readJson(req)).status, 30);
    if (!["Requested", "Scheduled", "Completed", "Cancelled"].includes(status)) return sendJson(res, 422, { error: "Invalid imaging status" }, requestId);
    db.prepare("UPDATE imaging_orders SET status = ?, updated_at = ? WHERE uuid = ?").run(status, now(), uuid);
    if (status === "Cancelled") removeInvoiceItem(uuid);
    audit(authenticatedUser.initials, "Imaging status updated", order.service_name, status, requestId);
    return sendJson(res, 200, { workflow: workflowState(order.patient_uuid), audit: listAudit() }, requestId);
  }
  if (req.method === "POST" && url.pathname === "/api/encounters") {
    ...
    const uuid = randomUUID();
    const timestamp = now();
    const clinicData = body.clinicData && typeof body.clinicData === "object" && !Array.isArray(body.clinicData) ? body.clinicData : {};
    const clinicDataJson = JSON.stringify(clinicData);
    if (clinicDataJson.length > 12000) return sendJson(res, 422, { error: "Clinic workflow data is too large" }, requestId);
    db.exec("BEGIN");
    try {
      db.prepare(`INSERT INTO encounters (uuid, patient_uuid, status, encounter_type, priority, department, vitals_json, subjective, objective, diagnosis, icd10, plan, follow_up, disposition, summary, clinic_data_json, clinician, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(uuid, patientUuid, "signed", text(body.encounterType || "Outpatient", 60), text(body.priority || "Routine", 40), text(body.department || "General outpatient", 80), JSON.stringify(body.vitals || {}), text(body.subjective, 5000), text(body.objective, 5000), text(body.diagnosis, 500), text(body.icd10, 30), text(body.plan, 5000), text(body.followUp, 500), text(body.disposition || "Home", 80), text(body.summary, 12000), clinicDataJson, authenticatedUser.displayName, timestamp, timestamp);
      ...
      db.prepare("UPDATE imaging_orders SET encounter_uuid = ?, updated_at = ? WHERE patient_uuid = ? AND encounter_uuid IS NULL").run(uuid, timestamp, patientUuid);
      ...
      db.prepare("UPDATE arrivals SET status = 'Completed', completed_at = ? WHERE patient_uuid = ? AND status IN ('Waiting', 'In progress')").run(timestamp, patientUuid);
    ...
    return sendJson(res, 201, { encounter: { uuid, status: "signed", createdAt: timestamp, clinicData }, audit: listAudit() }, requestId);
  }
